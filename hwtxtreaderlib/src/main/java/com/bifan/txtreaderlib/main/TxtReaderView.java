...
public class TxtReaderView extends TxtReaderBaseView {
    ...
    // 设置TxtReaderView 加载成功之前 默认背景颜色，不设置 为 白色
    public void setDefaultBgColor(int color) {
        readerContext.getTxtConfig().backgroundColor = color;
    }
     ...
     * 跳转到指定章节
     */
    public void jumpToIndex(final int index) {
        List<IChapter> chapters = getChapters();
        refreshTag(1, 1, 1);
        IChapter nextChapter = chapters.get(index);
        loadFromProgress(nextChapter.getStartParagraphIndex(), 0);
    }
